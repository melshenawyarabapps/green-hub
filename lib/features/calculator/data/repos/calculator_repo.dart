...
class CalculatorRepo {
  ...
  CalculatorModel? onNumberPressed(
    ...
    if (calculatorModel.type.isCurrency &&
        (selectedCardIndex < 2 && numberModel.type.isNumber)) {
      return calculatorModel;
    }
    ...
    switch (numberModel.type) {
      ...
      case NumberType.delete:
        if (price != 0 && selectedCard.price.length != 1) {
          ...
          selectedCard = selectedCard.copyWith(
            price:
                newCalculatorModel.type.isCurrency&&selectedCardIndex<2
      ...
      case NumberType.ac:
        ...
        for (int index = 0; index < newCalculatorModel.cards.length; index++) {
          newCards.add(
            newCalculatorModel.cards[index].copyWith(
              price:
                  newCalculatorModel.type.isCurrency&&index<2
                      ? LocaleKeys.selectCurrency.tr()
                      : '00.0',
              isSelected: index == 0,
            ),
        ...
        return newCalculatorModel.copyWith(cards: newCards, totalPrice: '00.0');
      case NumberType.equal:
        if (!newCalculatorModel.type.isCurrency) {
          if (newCalculatorModel.cards[1].price == '00.0') {
            return newCalculatorModel;
          }
        }
